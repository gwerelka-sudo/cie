...
public final class CIECommand {
    ...
    private static int setComponent(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ...
        try {
            ...
            NbtElement nbt = StringNbtReader.fromOps(NbtOps.INSTANCE).read(snbtValue);
    ...
    private static int getFullItemInfo(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ...
        try {
            ...
            nbt.accept(writer);
            sendCopyableRaw(ctx.getSource(), writer.getString());
            return 1;
        } catch (Exception e) {
            sendLangFeedback(ctx.getSource(), "component_parse_error", e.getMessage());
            return 0;
    ...
    private static int getTooltipHideAll(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        TooltipDisplayComponent component = TooltipDisplayUtil.getOrCreate(stack);
    ...
    private static int clearTooltip(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ...
        stack.remove(DataComponentTypes.TOOLTIP_DISPLAY);
        syncHandItem(player, stack);
    ...
    private static int setTooltipHidden(CommandContext<FabricClientCommandSource> ctx, String key, boolean hidden) throws CommandSyntaxException {
        ...
        TooltipDisplayComponent updated = TooltipDisplayUtil.withHiddenComponent(TooltipDisplayUtil.getOrCreate(stack), target, hidden);
    ...
    private static int setTooltipHideAll(CommandContext<FabricClientCommandSource> ctx, boolean hide) throws CommandSyntaxException {
        ...
        TooltipDisplayComponent updated = TooltipDisplayUtil.withHideWholeTooltip(TooltipDisplayUtil.getOrCreate(stack), hide);
        TooltipDisplayUtil.save(stack, updated);
    ...
    private static int openMouseHistory(CommandContext<FabricClientCommandSource> ctx) {
        ...
        // синхронно. Поэтому открытие откладываем на следующий тик через
        // client.execute — тогда наш setScreen срабатывает уже после
        // того, как ChatScreen закроет сам себя, и остаётся активным.
        MinecraftClient client = MinecraftClient.getInstance();
        client.execute(() -> {
            try {
                ...
                client.setScreen(new com.cie.screen.MouseHistoryScreen(handler, player.getInventory()));
            } catch (Throwable t) {
                t.printStackTrace();
            }
        });
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> weaponNode() {
        return ClientCommandManager.literal("weapon")
                .then(ClientCommandManager.literal("clear").executes(CIECommand::clearWeapon))
                .then(ClientCommandManager.literal("damagePerAttack")
                        .then(ClientCommandManager.literal("get").executes(CIECommand::getWeaponDamagePerAttack))
                        .then(ClientCommandManager.literal("set")
                                .then(ClientCommandManager.argument("dmg", IntegerArgumentType.integer())
                                        .executes(CIECommand::setWeaponDamagePerAttack)))
                        .then(ClientCommandManager.literal("reset")
                                .executes(ctx -> setWeaponDamagePerAttackValue(ctx, 1))))
                .then(ClientCommandManager.literal("disablingSeconds")
                        .then(ClientCommandManager.literal("get").executes(CIECommand::getWeaponDisablingSeconds))
                        .then(ClientCommandManager.literal("set")
                                .then(ClientCommandManager.argument("secs", FloatArgumentType.floatArg(0.0f))
                                        .executes(CIECommand::setWeaponDisablingSeconds)))
                        .then(ClientCommandManager.literal("reset")
                                .executes(ctx -> setWeaponDisablingSecondsValue(ctx, 0.0f))));
    }

    private static int setWeaponDamagePerAttackValue(CommandContext<FabricClientCommandSource> ctx, int dmg) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        WeaponUtil.setDamagePerAttack(stack, dmg);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "weapon_damage_set", dmg);
        return 1;
    }

    private static int setWeaponDisablingSecondsValue(CommandContext<FabricClientCommandSource> ctx, float secs) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        WeaponUtil.setDisablingSeconds(stack, secs);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "weapon_disabling_set", secs);
        return 1;
    }

    private static int getWeaponDamagePerAttack(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "weapon_damage_status", WeaponUtil.getDamagePerAttack(stack));
        return 1;
    }

    private static int setWeaponDamagePerAttack(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        int dmg = IntegerArgumentType.getInteger(ctx, "dmg");
        WeaponUtil.setDamagePerAttack(stack, dmg);
    ...
    private static int getWeaponDisablingSeconds(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "weapon_disabling_status", WeaponUtil.getDisablingSeconds(stack));
        return 1;
    }

    private static int setWeaponDisablingSeconds(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
    ...
    private static int clearWeapon(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        WeaponUtil.clearWeapon(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "weapon_cleared");
        return 1;
    }

    // ================================================================
    //  /cie breakSound ...  (BREAK_SOUND)
    // ================================================================

    private static LiteralArgumentBuilder<FabricClientCommandSource> breakSoundNode() {
        return ClientCommandManager.literal("breakSound")
                .then(ClientCommandManager.literal("get").executes(CIECommand::getBreakSound))
                .then(ClientCommandManager.literal("set")
                        .then(ClientCommandManager.argument("id", IdentifierArgumentType.identifier())
                                .suggests(SOUND_EVENT_SUGGESTIONS)
                                .executes(CIECommand::setBreakSound)))
                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetBreakSound));
    }

    private static int resetBreakSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BreakSoundUtil.removeBreakSound(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "break_sound_reset");
        return 1;
    }

    private static int getBreakSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        RegistryEntry<SoundEvent> sound = BreakSoundUtil.getBreakSound(stack);
        sendLangFeedback(ctx.getSource(), "break_sound_status", sound == null ? "нет" : soundId(sound));
        return 1;
    }

    private static int setBreakSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        BreakSoundUtil.setBreakSound(stack, sound);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "break_sound_set", soundId.toString());
        return 1;
    }

    private static String soundId(RegistryEntry<SoundEvent> sound) {
        return sound.getKey().map(key -> key.getValue().toString()).orElse(sound.value().id().toString());
    }
    ...
    private static DyeColor parseDyeColor(String name) throws CommandSyntaxException {
        DyeColor color = DyeColor.byId(name.toLowerCase(Locale.ROOT), null);
        if (color == null) {
    ...
    private static void openStoragePage(FabricClientCommandSource source, int pageIndex) {
        // Тот же гочу-фикс, что и у mouseHistory: открываем отложенно
        // через client.execute, иначе ChatScreen сам закроется после
        // выполнения команды и затрёт наш только что открытый экран.
        MinecraftClient client = MinecraftClient.getInstance();
        RegistryWrapper.WrapperLookup registries = getRegistries();
        client.execute(() -> {
            try {
    ...
    // ================================================================
    //  /cie blockAttacks ...  (BLOCKS_ATTACKS)
    // ================================================================

    private static LiteralArgumentBuilder<FabricClientCommandSource> blockAttacksNode() {
        return ClientCommandManager.literal("blockAttacks")
                .then(ClientCommandManager.literal("clear").executes(CIECommand::clearBlockAttacks))
                .then(ClientCommandManager.literal("blockDelaySeconds")
                        .then(ClientCommandManager.literal("set")
                                .then(ClientCommandManager.argument("secs", IntegerArgumentType.integer())
                                        .executes(CIECommand::setBlockDelaySeconds)))
                        .then(ClientCommandManager.literal("get").executes(CIECommand::getBlockDelaySeconds))
                        .then(ClientCommandManager.literal("reset").executes(CIECommand::resetBlockDelaySeconds)))
                .then(ClientCommandManager.literal("blockSound")
                        .then(ClientCommandManager.literal("set")
                                .then(ClientCommandManager.argument("sound", IdentifierArgumentType.identifier())
                                        .suggests(SOUND_EVENT_SUGGESTIONS)
                                        .executes(CIECommand::setBlockSound)))
                        .then(ClientCommandManager.literal("get").executes(CIECommand::getBlockSound))
                        .then(ClientCommandManager.literal("reset").executes(CIECommand::resetBlockSound)))
                .then(ClientCommandManager.literal("bypassedBy")
                        .then(ClientCommandManager.literal("set")
                                .then(ClientCommandManager.argument("by", IdentifierArgumentType.identifier())
                                        .suggests(DAMAGE_TYPE_TAG_SUGGESTIONS)
                                        .executes(CIECommand::setBypassedBy)))
                        .then(ClientCommandManager.literal("get").executes(CIECommand::getBypassedBy))
                        .then(ClientCommandManager.literal("reset").executes(CIECommand::resetBypassedBy)))
                .then(ClientCommandManager.literal("damageReducations")
                        .then(ClientCommandManager.literal("clear").executes(CIECommand::clearDamageReducations))
                        .then(ClientCommandManager.literal("base")
                                .then(ClientCommandManager.literal("set")
                                        .then(ClientCommandManager.argument("val", FloatArgumentType.floatArg())
                                                .executes(CIECommand::setDamageReducBase)))
                                .then(ClientCommandManager.literal("get").executes(CIECommand::getDamageReducBase))
                                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetDamageReducBase)))
                        .then(ClientCommandManager.literal("factor")
                                .then(ClientCommandManager.literal("set")
                                        .then(ClientCommandManager.argument("val", FloatArgumentType.floatArg())
                                                .executes(CIECommand::setDamageReducFactor)))
                                .then(ClientCommandManager.literal("get").executes(CIECommand::getDamageReducFactor))
                                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetDamageReducFactor)))
                        .then(ClientCommandManager.literal("horizontalBlockingAngle")
                                .then(ClientCommandManager.literal("set")
                                        .then(ClientCommandManager.argument("val", FloatArgumentType.floatArg())
                                                .executes(CIECommand::setDamageReducAngle)))
                                .then(ClientCommandManager.literal("get").executes(CIECommand::getDamageReducAngle))
                                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetDamageReducAngle)))
                        .then(ClientCommandManager.literal("type")
                                .then(ClientCommandManager.literal("set")
                                        .then(ClientCommandManager.argument("type", IdentifierArgumentType.identifier())
                                                .suggests(DAMAGE_TYPE_TAG_SUGGESTIONS)
                                                .executes(CIECommand::setDamageReducType)))
                                .then(ClientCommandManager.literal("get").executes(CIECommand::getDamageReducType))
                                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetDamageReducType))))
                .then(ClientCommandManager.literal("disableCooldownScale")
                        .then(ClientCommandManager.literal("get").executes(CIECommand::getDisableCooldownScale))
                        .then(ClientCommandManager.literal("set")
                                .then(ClientCommandManager.argument("scale", IntegerArgumentType.integer())
                                        .executes(CIECommand::setDisableCooldownScale)))
                        .then(ClientCommandManager.literal("reset").executes(CIECommand::resetDisableCooldownScale)))
                .then(ClientCommandManager.literal("disabledSound")
                        .then(ClientCommandManager.literal("get").executes(CIECommand::getDisabledSound))
                        .then(ClientCommandManager.literal("set")
                                .then(ClientCommandManager.argument("sound", IdentifierArgumentType.identifier())
                                        .suggests(SOUND_EVENT_SUGGESTIONS)
                                        .executes(CIECommand::setDisabledSound)))
                        .then(ClientCommandManager.literal("reset").executes(CIECommand::resetDisabledSound)))
                .then(ClientCommandManager.literal("itemDamage")
                        .then(ClientCommandManager.literal("base")
                                .then(ClientCommandManager.literal("set")
                                        .then(ClientCommandManager.argument("val", FloatArgumentType.floatArg())
                                                .executes(CIECommand::setItemDamageBase)))
                                .then(ClientCommandManager.literal("get").executes(CIECommand::getItemDamageBase))
                                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetItemDamageBase)))
                        .then(ClientCommandManager.literal("factor")
                                .then(ClientCommandManager.literal("set")
                                        .then(ClientCommandManager.argument("val", FloatArgumentType.floatArg())
                                                .executes(CIECommand::setItemDamageFactor)))
                                .then(ClientCommandManager.literal("get").executes(CIECommand::getItemDamageFactor))
                                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetItemDamageFactor)))
                        .then(ClientCommandManager.literal("threshold")
                                .then(ClientCommandManager.literal("set")
                                        .then(ClientCommandManager.argument("val", FloatArgumentType.floatArg())
                                                .executes(CIECommand::setItemDamageThreshold)))
                                .then(ClientCommandManager.literal("get").executes(CIECommand::getItemDamageThreshold))
                                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetItemDamageThreshold))));
    }

    private static int clearBlockAttacks(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.clear(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "block_attacks_cleared");
        return 1;
    }

    private static int setBlockDelaySeconds(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        int secs = IntegerArgumentType.getInteger(ctx, "secs");
        BlockAttacksUtil.BlockDelaySeconds.set(stack, secs);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "block_delay_seconds_set", secs);
        return 1;
    }

    private static int getBlockDelaySeconds(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "block_delay_seconds_status", BlockAttacksUtil.BlockDelaySeconds.get(stack));
        return 1;
    }

    private static int resetBlockDelaySeconds(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.BlockDelaySeconds.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "block_delay_seconds_status", BlockAttacksUtil.BlockDelaySeconds.get(stack));
        return 1;
    }

    private static int setBlockSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        Identifier soundId = ctx.getArgument("sound", Identifier.class);
        BlockAttacksUtil.BlockSound.set(stack, soundId.toString());

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "block_sound_set", soundId.toString());
        return 1;
    }

    private static int getBlockSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "block_sound_status", BlockAttacksUtil.BlockSound.get(stack));
        return 1;
    }

    private static int resetBlockSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.BlockSound.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "block_sound_reset");
        return 1;
    }

    private static int setBypassedBy(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        String by = ctx.getArgument("by", Identifier.class).toString();
        BlockAttacksUtil.BypassedBy.set(stack, by);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "bypassed_by_set", by);
        return 1;
    }

    private static int getBypassedBy(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "bypassed_by_status", BlockAttacksUtil.BypassedBy.get(stack));
        return 1;
    }

    private static int resetBypassedBy(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.BypassedBy.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "bypassed_by_reset");
        return 1;
    }

    private static int clearDamageReducations(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.DamageReducations.clear(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_cleared");
        return 1;
    }

    private static int setDamageReducBase(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        BlockAttacksUtil.DamageReducations.Base.set(stack, val);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_base_set", val);
        return 1;
    }

    private static int getDamageReducBase(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "damage_reduc_base_status", BlockAttacksUtil.DamageReducations.Base.get(stack));
        return 1;
    }

    private static int resetDamageReducBase(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.DamageReducations.Base.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_base_reset");
        return 1;
    }

    private static int setDamageReducFactor(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        float val = FloatArgumentType.getFloat(ctx, "val");
        BlockAttacksUtil.DamageReducations.Factor.set(stack, val);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_factor_set", val);
        return 1;
    }

    private static int getDamageReducFactor(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "damage_reduc_factor_status", BlockAttacksUtil.DamageReducations.Factor.get(stack));
        return 1;
    }

    private static int resetDamageReducFactor(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.DamageReducations.Factor.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_factor_reset");
        return 1;
    }

    private static int setDamageReducAngle(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        float val = FloatArgumentType.getFloat(ctx, "val");
        BlockAttacksUtil.DamageReducations.HorizontalBlockingAngle.set(stack, val);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_angle_set", val);
        return 1;
    }

    private static int getDamageReducAngle(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "damage_reduc_angle_status", BlockAttacksUtil.DamageReducations.HorizontalBlockingAngle.get(stack));
        return 1;
    }

    private static int resetDamageReducAngle(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.DamageReducations.HorizontalBlockingAngle.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_angle_reset");
        return 1;
    }

    private static int setDamageReducType(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        String type = ctx.getArgument("type", Identifier.class).toString();
        BlockAttacksUtil.DamageReducations.Type.set(stack, type);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_type_set", type);
        return 1;
    }

    private static int getDamageReducType(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "damage_reduc_type_status", BlockAttacksUtil.DamageReducations.Type.get(stack));
        return 1;
    }

    private static int resetDamageReducType(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.DamageReducations.Type.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "damage_reduc_type_status", "");
        return 1;
    }

    private static int getDisableCooldownScale(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "disable_cooldown_scale_status", BlockAttacksUtil.DisableCooldownScale.get(stack));
        return 1;
    }

    private static int setDisableCooldownScale(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        BlockAttacksUtil.DisableCooldownScale.set(stack, scale);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "disable_cooldown_scale_set", scale);
        return 1;
    }

    private static int resetDisableCooldownScale(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.DisableCooldownScale.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "disable_cooldown_scale_reset");
        return 1;
    }

    private static int getDisabledSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "disabled_sound_status", BlockAttacksUtil.DisabledSound.get(stack));
        return 1;
    }

    private static int setDisabledSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        Identifier soundId = ctx.getArgument("sound", Identifier.class);
        BlockAttacksUtil.DisabledSound.set(stack, soundId.toString());

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "disabled_sound_set", soundId.toString());
        return 1;
    }

    private static int resetDisabledSound(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.DisabledSound.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "disabled_sound_reset");
        return 1;
    }

    private static int setItemDamageBase(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        float val = FloatArgumentType.getFloat(ctx, "val");
        BlockAttacksUtil.ItemDamage.Base.set(stack, val);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "item_damage_base_set", val);
        return 1;
    }

    private static int getItemDamageBase(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "item_damage_base_status", BlockAttacksUtil.ItemDamage.Base.get(stack));
        return 1;
    }

    private static int resetItemDamageBase(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.ItemDamage.Base.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "item_damage_base_reset");
        return 1;
    }

    private static int setItemDamageFactor(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        float val = FloatArgumentType.getFloat(ctx, "val");
        BlockAttacksUtil.ItemDamage.Factor.set(stack, val);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "item_damage_factor_set", val);
        return 1;
    }

    private static int getItemDamageFactor(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "item_damage_factor_status", BlockAttacksUtil.ItemDamage.Factor.get(stack));
        return 1;
    }

    private static int resetItemDamageFactor(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.ItemDamage.Factor.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "item_damage_factor_reset");
        return 1;
    }

    private static int setItemDamageThreshold(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        float val = FloatArgumentType.getFloat(ctx, "val");
        BlockAttacksUtil.ItemDamage.Threshold.set(stack, val);

        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "item_damage_threshold_set", val);
        return 1;
    }

    private static int getItemDamageThreshold(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        sendLangFeedback(ctx.getSource(), "item_damage_threshold_status", BlockAttacksUtil.ItemDamage.Threshold.get(stack));
        return 1;
    }

    private static int resetItemDamageThreshold(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);

        BlockAttacksUtil.ItemDamage.Threshold.reset(stack);
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "item_damage_threshold_reset");
        return 1;
    }
     ...
     * у openStoragePage: client.execute, иначе ChatScreen сам закроется
    ...
    private static void openItemContentsEditor(FabricClientCommandSource source, int rows, List<ItemStack> initial, String title,
        ...
        client.execute(() -> {
            try {
                ...
                client.setScreen(new com.cie.screen.ItemContentsScreen(handler, player.getInventory(), Text.literal(title), contents -> {
                    ClientPlayerEntity p = MinecraftClient.getInstance().player;
                    if (p == null) {
                        return;
                    }
