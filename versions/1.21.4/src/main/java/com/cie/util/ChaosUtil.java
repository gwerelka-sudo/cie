 ...
 * на набор "безопасных" компонентов (не ломающих клиент/рендер произвольным
 * мусором — в отличие, например, от ItemEnchantmentsComponent или
 * PotionContentsComponent, которые требуют валидных реестровых ссылок).
 *
 * overwrite=true — сначала полностью очищает все компоненты из этого набора,
 * затем ставит новый случайный хаос (т.е. каждый вызов даёт "чистый" повторный
 * рандом). overwrite=false (по умолчанию) — просто докидывает ещё один слой
 * случайных значений поверх текущих (без предварительной очистки).
 */
public final class ChaosUtil {
    ...
    /** Бессмысленные слоги/слова — не несут смысла ни на одном языке, просто "фан"-набор для рандомного текста. */
    ...
    /** Случайные "яркие" hex-цвета для рандомного name/lore — намеренно из разных частей спектра. */
    ...
    /** Шанс, что /cie chaos тронет имя (0.0-1.0). */
    private static final double NAME_CHANCE = 0.35;
    /** Шанс, что /cie chaos тронет лор (0.0-1.0). */
    ...
    /** Применяет случайный хаос к предмету. */
    public static void apply(ItemStack stack, boolean overwrite) {
        ...
        // dyed color / map color (случайный rgb)
        ...
        // custom model data (float, первый слот)
        CustomModelDataUtil.Floats.set(stack, List.of(RANDOM.nextFloat() * 10f));

        // glint override — случайно true/false
        stack.set(DataComponentTypes.ENCHANTMENT_GLINT_OVERRIDE, RANDOM.nextBoolean());

        // damage resistant (случайно ставим "огнестойкость" через уже
        // проверенный в проекте DamageResistantUtil, вместо гадания с
        // отдельным DataComponentTypes.FIRE_RESISTANT)
        if (RANDOM.nextBoolean()) {
            DamageResistantUtil.set(stack, "is_fire");
        } else {
            DamageResistantUtil.clear(stack);
        }

        // max stack size — случайное разумное значение
        stack.set(DataComponentTypes.MAX_STACK_SIZE, RANDOM.nextInt(1, 100));

        // custom name — с шансом NAME_CHANCE, случайные бессмысленные слова, разноцветные
        if (RANDOM.nextDouble() < NAME_CHANCE) {
            stack.set(DataComponentTypes.CUSTOM_NAME, randomGibberishLine());
        }

        // lore — с шансом LORE_CHANCE, от 1 до 4 строк бессмысленного текста, разноцветные
        ...
        // enchantable value (см. предупреждение у /cie edit enchantable в
        // CIECommand про сигнатуру EnchantableComponent)
        stack.set(DataComponentTypes.ENCHANTABLE, new net.minecraft.component.type.EnchantableComponent(RANDOM.nextInt(1, 51)));
    }

    /** Собирает одну строку из 2-5 случайных бессмысленных слов, каждое слово — свой случайный цвет. */
    ...
    /** Убирает все компоненты, которые /cie chaos умеет рандомизировать (используется при overwrite=true). */
