...
public final class VillagerDataUtil {
    ...
    // ============================================================
    //  Хелперы: в 1.21.4 у NbtCompound.getString/getBoolean/getInt/
    //  getLong/getFloat НЕТ перегрузки (key, fallback) — только
    //  однопараметрическая версия со встроенным дефолтом ("" / false / 0).
    //  Кастомные дефолты (не "" / 0 / false) эмулируем через contains().
    // ============================================================

    private static String getStringOr(NbtCompound c, String key, String def) {
        return c.contains(key) ? c.getString(key) : def;
    }

    private static int getIntOr(NbtCompound c, String key, int def) {
        return c.contains(key) ? c.getInt(key) : def;
    }

    private static boolean getBooleanOr(NbtCompound c, String key, boolean def) {
        return c.contains(key) ? c.getBoolean(key) : def;
    }

    private static long getLongOr(NbtCompound c, String key, long def) {
        return c.contains(key) ? c.getLong(key) : def;
    }

    private static float getFloatOr(NbtCompound c, String key, float def) {
        return c.contains(key) ? c.getFloat(key) : def;
    }
    ...
    public static String getProfession(ItemStack stack) {
        return getStringOr(getVillagerDataCompound(stack), PROFESSION_KEY, DEFAULT_PROFESSION);
    }
    ...
    public static String getBiome(ItemStack stack) {
        return getStringOr(getVillagerDataCompound(stack), TYPE_KEY, DEFAULT_BIOME);
    }
    ...
    public static int getLevel(ItemStack stack) {
        return getIntOr(getVillagerDataCompound(stack), LEVEL_KEY, DEFAULT_LEVEL);
    }
    ...
    public static boolean getWilling(ItemStack stack) {
        return getBooleanOr(EntitySettingsUtil.getRoot(stack), WILLING_KEY, false);
    }
    ...
    public static long getLastRestock(ItemStack stack) {
        return getLongOr(EntitySettingsUtil.getRoot(stack), LAST_RESTOCK_KEY, 0L);
    }
    ...
    private static NbtList getRecipes(ItemStack stack) {
        return EntitySettingsUtil.getRoot(stack).getCompound(OFFERS_KEY).getList(RECIPES_KEY, NbtCompound.COMPOUND_TYPE);
    }

    private static void saveRecipes(ItemStack stack, NbtList recipes) {
        EntitySettingsUtil.mutate(stack, root -> {
            NbtCompound offers = root.getCompound(OFFERS_KEY);
    ...
    public static NbtCompound getTrade(ItemStack stack, int index0) {
        ...
        return recipes.getCompound(index0);
    }
    ...
    private static void mutateTrade(ItemStack stack, int index1, Consumer<NbtCompound> mutator) {
        ...
        NbtCompound trade = recipes.getCompound(index0).copy();
    ...
    public static boolean getRewardExp(ItemStack stack, int index1) {
        ...
        return trade != null && getBooleanOr(trade, "rewardExp", true);
    }
    ...
    public static int getMaxUses(ItemStack stack, int index1) {
        NbtCompound trade = getTrade(stack, index1 - 1);
        return trade != null ? getIntOr(trade, "maxUses", 0) : 0;
    }
    ...
    public static int getUses(ItemStack stack, int index1) {
        NbtCompound trade = getTrade(stack, index1 - 1);
        return trade != null ? getIntOr(trade, "uses", 0) : 0;
    }
    ...
    public static int getXp(ItemStack stack, int index1) {
        NbtCompound trade = getTrade(stack, index1 - 1);
        return trade != null ? getIntOr(trade, "xp", 0) : 0;
    }
    ...
    public static int getPriceMultiplier(ItemStack stack, int index1) {
        NbtCompound trade = getTrade(stack, index1 - 1);
        return trade != null ? (int) getFloatOr(trade, "priceMultiplier", 0f) : 0;
    }
    ...
    public static int getSpecialPrice(ItemStack stack, int index1) {
        NbtCompound trade = getTrade(stack, index1 - 1);
        return trade != null ? getIntOr(trade, "specialPrice", 0) : 0;
    }
    ...
    public static int getDemand(ItemStack stack, int index1) {
        NbtCompound trade = getTrade(stack, index1 - 1);
        return trade != null ? getIntOr(trade, "demand", 0) : 0;
    }
    ...
    public static ItemStack getTradeItem(ItemStack stack, int index1, String field, RegistryWrapper.WrapperLookup registries) {
        ...
        var ops = registries.getOps(NbtOps.INSTANCE);
        return ItemStack.CODEC.parse(ops, trade.getCompound(field)).result().orElse(ItemStack.EMPTY);
    }
