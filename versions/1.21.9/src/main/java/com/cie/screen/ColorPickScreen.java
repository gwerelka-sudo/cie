 ...
 * RenderPipeline, не RenderLayer-функцию, с 1.21.6+), NativeImage.getColor
 * (стал private — используем публичный getColorArgb) и drawBorder,
 * которого в этой версии DrawContext вообще нет (используем
...
public class ColorPickScreen extends Screen {
    ...
    private void renderMagnifier(DrawContext context, int mouseX, int mouseY, int argb) {
        ...
        // drawBorder в этой версии не существует — рисуем рамку через drawStrokedRectangle.
        context.drawStrokedRectangle(sx, sy, boxW, boxH, UiColorUtil.get("colorPick.magnifierBorder"));

        int swatchX = sx + pad;
        int swatchY = sy + pad;
        context.fill(swatchX, swatchY, swatchX + swatchSize, swatchY + swatchSize, 0xFF000000 | (argb & 0xFFFFFF));
        context.drawStrokedRectangle(swatchX, swatchY, swatchSize, swatchSize, UiColorUtil.get("colorPick.swatchBorder"));
