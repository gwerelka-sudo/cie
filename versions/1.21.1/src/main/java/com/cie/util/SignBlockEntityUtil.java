 ...
 * back_text.
 *
 * [1.21.4] DyeColor.getId() в этой версии возвращает int (числовой индекс
 * цвета 0-15), а не String — строковая форма/лукап переехали в getId():String
 * и byId(String, fallback) только в 1.21.8+. Здесь, как и было исторически,
 * строковое имя цвета — это asString() (сериализационное имя), а обратный
 * поиск по строке — DyeColor.byName(String, DyeColor).
 */
public final class SignBlockEntityUtil {
    ...
    private static void saveRoot(ItemStack stack, NbtCompound root) {
        ...
        withId.putString("id", Registries.BLOCK_ENTITY_TYPE.getId(BlockEntityType.SIGN).toString());
    ...
    private static NbtCompound getSide(NbtCompound root, String sideKey) {
        return root.getCompound(sideKey);
    }
    ...
    private static NbtCompound ensureScalarDefaults(NbtCompound side) {
        if (!side.contains(COLOR_KEY)) {
            side.putString(COLOR_KEY, DyeColor.BLACK.asString());
        }
    ...
    public static boolean isWaxed(ItemStack stack) {
        return getRoot(stack).getBoolean(WAXED_KEY);
    }
    ...
    public static boolean isGlowing(ItemStack stack, String side) {
        ...
        return sideNbt.getBoolean(GLOWING_KEY);
    }
    ...
    public static DyeColor getBaseColor(ItemStack stack, String side) {
        NbtCompound sideNbt = ensureScalarDefaults(getSide(getRoot(stack), side));
        // ensureScalarDefaults уже гарантирует наличие COLOR_KEY, поэтому
        // однопараметрический getString(key) (без fallback-перегрузки,
        // которой в этой версии у NbtCompound нет) достаточен.
        return DyeColor.byName(sideNbt.getString(COLOR_KEY), DyeColor.BLACK);
    }

    public static void setBaseColor(ItemStack stack, String side, DyeColor color) {
        NbtCompound root = getRoot(stack);
        NbtCompound sideNbt = ensureScalarDefaults(getSide(root, side));
        sideNbt.putString(COLOR_KEY, color.asString());
        root.put(side, sideNbt);
        saveRoot(stack, root);
    }
    ...
    public static List<String> getLines(ItemStack stack, String side, RegistryWrapper.WrapperLookup registries) {
        ...
        NbtList messages = sideNbt.getList(MESSAGES_KEY, NbtCompound.COMPOUND_TYPE);
