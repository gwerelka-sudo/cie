...
public final class CIECommand {
    ...
    private static int setComponent(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ...
        try {
            ...
            NbtElement nbt = StringNbtReader.parse(snbtValue);
    ...
    private static int getTrim(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ...
        String patternId = trim.getPattern().getKey().map(k -> k.getValue().toString()).orElse("?");
        String materialId = trim.getMaterial().getKey().map(k -> k.getValue().toString()).orElse("?");
    ...
    private static <T> RegistryEntry<T> resolveEntry(RegistryKey<Registry<T>> registryKey, String idStr) throws CommandSyntaxException {
        ...
        RegistryWrapper.WrapperLookup registries = getRegistries();
        RegistryWrapper.Impl<T> wrapper = registries.getOptionalWrapper(registryKey)
    ...
    private static int giveItem(CommandContext<FabricClientCommandSource> ctx, int count, String componentsSnbt) throws CommandSyntaxException {
        ...
        ItemStack stack;
        try {
            stack = buildStackHelper(item, count, componentsSnbt, getRegistries());
        } catch (Exception e) {
            sendLangFeedback(ctx.getSource(), "component_parse_error", e.getMessage());
            return 0;
        }
    ...
    private static LiteralArgumentBuilder<FabricClientCommandSource> customModelDataNode() {
        return ClientCommandManager.literal("customModelData")
                .then(ClientCommandManager.literal("get").executes(CIECommand::getCmdCustomModelData))
                .then(ClientCommandManager.literal("set")
                        .then(ClientCommandManager.argument("value", IntegerArgumentType.integer())
                                .executes(CIECommand::setCmdCustomModelData)))
                .then(ClientCommandManager.literal("reset").executes(CIECommand::resetCmdCustomModelData));
    }

// -- custom model data (int) --

    private static int getCmdCustomModelData(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ItemStack stack = requireItem(ctx);
        CustomModelDataComponent component = stack.get(DataComponentTypes.CUSTOM_MODEL_DATA);
        String valueStr = (component != null) ? component.toString() : "none";
        sendLangFeedback(ctx.getSource(), "cmd_custom_model_data_status", valueStr);
        return 1;
    }

    private static int setCmdCustomModelData(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "cmd_custom_model_data_set", String.valueOf(value));
        return 1;
    }

    private static int resetCmdCustomModelData(CommandContext<FabricClientCommandSource> ctx) throws CommandSyntaxException {
        ClientPlayerEntity player = requireCreativePlayer(ctx);
        ItemStack stack = player.getMainHandStack();
        if (stack.isEmpty()) throw createException("no_item");
        UndoUtil.pushSnapshot(player, stack);
        ...
        syncHandItem(player, stack);
        sendLangFeedback(ctx.getSource(), "cmd_custom_model_data_reset");
        return 1;
    }
    ...
    private static int setEquipmentNew(CommandContext<FabricClientCommandSource> ctx, String slotLiteral, String componentsSnbt) throws CommandSyntaxException {
        ...
        ItemStack toEquip;
        try {
            toEquip = buildStackHelper(newItem, count, componentsSnbt, getRegistries());
        } catch (Exception e) {
            sendLangFeedback(ctx.getSource(), "component_parse_error", e.getMessage());
            return 0;
        }
    ...
    private static int setTradeItemNew(CommandContext<FabricClientCommandSource> ctx, String field, String componentsSnbt) throws CommandSyntaxException {
        ...
        ItemStack toSet;
        try {
            toSet = buildStackHelper(newItem, count, componentsSnbt, getRegistries());
        } catch (Exception e) {
            sendLangFeedback(ctx.getSource(), "component_parse_error", e.getMessage());
            return 0;
        }
    ...
    private static ItemStack buildStackHelper(Item item, int count, String componentsSnbt, RegistryWrapper.WrapperLookup registries) throws Exception {
        ItemStack stack = new ItemStack(item, count);
        if (componentsSnbt != null && !componentsSnbt.trim().isEmpty()) {
            String fullSnbt = "{id:\"" + Registries.ITEM.getId(item) + "\",count:" + count + ",components:" + componentsSnbt + "}";
            return ImportUtil.fromSnbt(fullSnbt, registries);
        }
        return stack;
    }
    ...
    private static int addContainerItemNew(CommandContext<FabricClientCommandSource> ctx, int count, String componentsSnbt) throws CommandSyntaxException {
        ...
        ItemStack toAdd;
        try {
            toAdd = buildStackHelper(newItem, count, componentsSnbt, getRegistries());
        } catch (Exception e) {
            sendLangFeedback(ctx.getSource(), "component_parse_error", e.getMessage());
            return 0;
        }
