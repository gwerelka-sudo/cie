...
public class armorStandEditScreen extends Screen {
    ...
    private void initEquipOverlay() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal(CIELang.get("armorstand_menu_equip_set_new_item")), b -> {
            ...
            try {
                ItemStack toEquip = new ItemStack(item, 1);
    ...
    private void openStoragePicker() {
        ...
        client.execute(() -> {
            ...
            try {
                ...
                StorageScreenHandler handler =
                        new StorageScreenHandler(player.getInventory(), pageIndex, page);
                client.setScreen(new StorageScreen(handler, player.getInventory(), registries));
            } catch (Throwable t) {
                t.printStackTrace();
            }
        });
     ...
     */
    /**
     * [1.21.4] В этой версии нет EntityRenderState.light/.shadowPieces/.outlineColor
     * и нет DrawContext.addEntity(...) — превью сущности в GUI рисуется другим
     * паттерном: EntityRenderDispatcher.render(entity, x, y, z, tickProgress,
     * matrices, vertexConsumers, light), который сам вызывает
     * getAndUpdateRenderState внутри (см. декомпил InventoryScreen#drawEntity
     * для этой версии — свет передаётся параметром в render(), не полем
     * стейта; аутлайн отключать не нужно, GUI-рендер сам не рисует аутлайны
     * если не попросить явно, а EntityRenderState.hasOutline тут недоступен
     * до создания стейта — трогать нечего).
     *
     * Копия структуры InventoryScreen.drawEntity с той же сутевой разницей,
     * что и раньше: ванильный метод жёстко высчитывает bodyYaw/pitch превью
     * из курсора мыши по формуле atan((center - mouse) / 40), что (а) игнорирует
     * entity.setYaw()/setPitch() для наших целей и (б) ограничивает угол
     * небольшим диапазоном. Здесь вместо формулы курсора подставляем
     * накопленные {@link #previewYaw}/{@link #previewPitch} напрямую в саму
     * сущность (bodyYaw/setYaw/setPitch/headYaw/prevHeadYaw), без ограничения
     * диапазона — полный оборот по обеим осям, управляемый только драгом по
     * превью.
     */
    private void drawArmorStandPreview(DrawContext context, int x1, int y1, int x2, int y2, int size, LivingEntity entity) {
        ...
        // [1.21.4] Поле называется prevHeadYaw, а не lastHeadYaw (переименовано
        // в более поздних версиях) — на эту версию lastHeadYaw не существует.
        float savedPrevHeadYaw = entity.prevHeadYaw;
        float savedHeadYaw = entity.headYaw;

        entity.bodyYaw = 180.0F + previewYaw;
        entity.setYaw(180.0F + previewYaw);
        ...
        float adjustedSize = (float) size / scale;

        context.getMatrices().push();
        context.getMatrices().translate((double) centerX, (double) centerY, 50.0);
        context.getMatrices().scale(adjustedSize, adjustedSize, -adjustedSize);
        context.getMatrices().translate(offset.x, offset.y, offset.z);
        context.getMatrices().multiply(rotation);
        context.draw();
        // [1.21.4] Нет метода enableGuiShaderLighting() — как и в ванильном
        // InventoryScreen#drawEntity для этой версии, здесь вызывается
        // enableGuiDepthLighting() (тот же метод, что и после рендера ниже,
        // просто переключает освещение в "3D"-режим перед рисованием сущности).
        DiffuseLighting.enableGuiDepthLighting();

        EntityRenderDispatcher dispatcher = MinecraftClient.getInstance().getEntityRenderDispatcher();
        dispatcher.setRotation(pitchRotation.conjugate(new Quaternionf()).rotateY(3.1415927F));
        dispatcher.setRenderShadows(false);
        var immediate = MinecraftClient.getInstance().getBufferBuilders().getEntityVertexConsumers();
        dispatcher.render(entity, 0.0, 0.0, 0.0, 0.0F, 1.0F, context.getMatrices(), immediate, 15728880);
        immediate.draw();
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        // Оставлено на случай, если движок всё же вызовет этот метод сам
        // (например, на других сборках) — делегирует в тот же общий
        // метод, который polling вызывает из render().
        if (dragging || draggingPreview) {
            cie$pollDrag(mouseX, mouseY);
            return true;
        }
    ...
    /**
     * [1.21.4] Общая логика драга панели/превью, вызывается КАЖДЫЙ КАДР
     * из render() пока зажата ЛКМ (см. комментарий в начале render) — а
     * не через отдельные события mouseDragged, которые в этой версии не
     * диспетчеризуются движком для этого класса.
     */
    private void cie$pollDrag(double mouseX, double mouseY) {
        if (dragging) {
            panelX = Math.max(0, Math.min(this.width - PANEL_W, (int) mouseX - dragOffX));
            panelY = Math.max(0, Math.min(this.height - PANEL_H, (int) mouseY - dragOffY));
            rebuild();
            return;
        }
        if (draggingPreview) {
            double dx = mouseX - lastPreviewDragX;
            double dy = mouseY - lastPreviewDragY;
            lastPreviewDragX = mouseX;
            lastPreviewDragY = mouseY;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        // [1.21.4] mouseDragged/mouseReleased НЕ вызываются движком для
        // этого класса — та же причина, что и в ChatScreenMixin
        // (mouseDragged/mouseReleased физически не переопределены нигде
        // в цепочке Screen -> AbstractParentElement, чисто default из
        // интерфейса ParentElement — переопределение ниже компилируется,
        // но реально не диспетчеризуется). Драг эмулируется здесь, через
        // прямой опрос состояния ЛКМ каждый кадр.
        long cie$windowHandle = MinecraftClient.getInstance().getWindow().getHandle();
        boolean cie$mouseDown = org.lwjgl.glfw.GLFW.glfwGetMouseButton(cie$windowHandle, org.lwjgl.glfw.GLFW.GLFW_MOUSE_BUTTON_LEFT) == org.lwjgl.glfw.GLFW.GLFW_PRESS;
        if (cie$mouseDown && (dragging || draggingPreview)) {
            cie$pollDrag(mouseX, mouseY);
        } else if (!cie$mouseDown && (dragging || draggingPreview)) {
            dragging = false;
            draggingPreview = false;
        }
        ...
        context.drawBorder(panelX, panelY, PANEL_W, PANEL_H, UiColorUtil.get("armorStandMenu.panelBorder"));
    ...
    private class AxisSlider extends SliderWidget {
        ...
        AxisSlider(int x, int y, int w, int h, String label, float degrees, ArmorStandDataUtil.Axis axis) {
            ...
            // super(...) may have called updateMessage()/applyValue() before
            // label/axis were assigned (they're still null/uninitialized at
            // that point) — refresh the message now that everything is set,
            // so the displayed text is correct from the first frame.
            updateMessage();
        }
        ...
        protected void updateMessage() {
            // Guard against the super() constructor invoking this before
            // 'label' is assigned — without this, that early call would either
            // NPE or (if swallowed) silently skip setting the message.
        ...
        protected void applyValue() {
            // Guard against the super() constructor invoking this before
            // 'axis' is assigned — this was the actual cause of the slider
            // snapping to 0.0: an early spurious applyValue() call was
            // overwriting the pose with a stale/default value right after
            // construction, before the real drag-driven calls ever happened.
            if (!initialized) return;
        ...
        public boolean mouseReleased(double mouseX, double mouseY, int button) {
            ...
            // [1.21.4] Старая система ввода: mouseReleased(double, double, int).
            rebuild();
            return result;
        }
