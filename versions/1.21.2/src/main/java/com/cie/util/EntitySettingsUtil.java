 ...
 * механизм, что и `minecraft:block_entity_data` у таблички, только тип
 * — {@code EntityType<?>}, а не {@code BlockEntityType<?>}).
 *
 ...
 *
 * [1.21.5] В этой версии minecraft:entity_data — компонент {@code NbtComponent}
 * (не типобезопасный TypedEntityData, появившийся позже). Тип сущности хранится
 * ВНУТРИ самого NBT как строковый ключ "id" (resource location), а не отдельно
 * от NBT. getRoot()/saveRoot() ниже эмулируют старый TypedEntityData-интерфейс
 * (root без "id", тип отдельно) поверх этого сырого формата, чтобы остальной
 * класс не пришлось трогать.
 *
 * [1.21.4] NbtCompound/NbtList в этой версии — СТАРЫЙ API (build.4), у геттеров
 * НЕТ перегрузки с явным дефолтным значением (не getBoolean(key, def), а только
 * getBoolean(key), возвращающий примитивный дефолт 0/0.0/""/false сам по себе).
 * Аналогично NbtList.getDouble(index)/getFloat(index)/getString(index) — один
 * аргумент. getCompoundOrEmpty/getListOrEmpty(1 arg) в этой версии тоже не
 * существуют — используются getCompound(key) и getList(key, type).
 *
...
public final class EntitySettingsUtil {
    ...
    /** Тип сущности, который реально будет заспавнен — из entity_data, либо (если не задан) выведенный из самого предмета-яйца. */
    ...
    public static void saveRoot(ItemStack stack, EntityType<?> type, NbtCompound root) {
        ...
        withId.putString("id", typeId.toString());
        stack.set(DataComponentTypes.ENTITY_DATA, NbtComponent.of(withId));
    }
    ...
    public static double getMotion(ItemStack stack, int axis) {
        NbtList motion = getRoot(stack).getList(MOTION_KEY, NbtElement.DOUBLE_TYPE);
        return motion.getDouble(axis);
    }

    public static void setMotion(ItemStack stack, int axis, double value) {
        mutate(stack, root -> {
            NbtList motion = ensureVectorD(root.getList(MOTION_KEY, NbtElement.DOUBLE_TYPE), 3);
    ...
    public static float getYaw(ItemStack stack) {
        return getRoot(stack).getList(ROTATION_KEY, NbtElement.FLOAT_TYPE).getFloat(0);
    }

    public static float getPitch(ItemStack stack) {
        return getRoot(stack).getList(ROTATION_KEY, NbtElement.FLOAT_TYPE).getFloat(1);
    }

    public static void setYaw(ItemStack stack, float yaw) {
        mutate(stack, root -> {
            NbtList rot = ensureVectorF(root.getList(ROTATION_KEY, NbtElement.FLOAT_TYPE), 2);
    ...
    public static boolean isCustomNameVisible(ItemStack stack) {
        return getRoot(stack).getBoolean(CUSTOM_NAME_VISIBLE_KEY);
    }
    ...
    public static List<String> getTags(ItemStack stack) {
        NbtList tags = getRoot(stack).getList(TAGS_KEY, NbtElement.STRING_TYPE);
    ...
    public static void addTag(ItemStack stack, String tag) {
        mutate(stack, root -> {
            NbtList tags = root.getList(TAGS_KEY, NbtElement.STRING_TYPE);
    ...
    public static boolean getInvulnerable(ItemStack stack) {
        return getRoot(stack).getBoolean(INVULNERABLE_KEY);
    }
    ...
    public static boolean getSilent(ItemStack stack) {
        return getRoot(stack).getBoolean(SILENT_KEY);
    }
    ...
    public static boolean getNoAI(ItemStack stack) {
        return getRoot(stack).getBoolean(NO_AI_KEY);
    }
    ...
    public static boolean getCanPickUpLoot(ItemStack stack) {
        return getRoot(stack).getBoolean(CAN_PICK_UP_LOOT_KEY);
    }
    ...
    public static boolean getNoGravity(ItemStack stack) {
        return getRoot(stack).getBoolean(NO_GRAVITY_KEY);
    }
    ...
    public static boolean getVisualFire(ItemStack stack) {
        return getRoot(stack).getBoolean(VISUAL_FIRE_KEY);
    }
    ...
    public static boolean getGlowing(ItemStack stack) {
        return getRoot(stack).getBoolean(GLOWING_KEY);
    }
    ...
    public static float getHealth(ItemStack stack) {
        NbtCompound root = getRoot(stack);
        return root.contains(HEALTH_KEY) ? root.getFloat(HEALTH_KEY) : 1.0f;
    }
    ...
    public static ItemStack getEquipment(ItemStack stack, String slot, RegistryWrapper.WrapperLookup registries) {
        NbtCompound equipment = getRoot(stack).getCompound(EQUIPMENT_KEY);
        if (!equipment.contains(slot)) {
            return ItemStack.EMPTY;
        }
        NbtCompound itemNbt = equipment.getCompound(slot);
        var ops = registries.getOps(NbtOps.INSTANCE);
    ...
    public static void setEquipment(ItemStack stack, String slot, ItemStack item, RegistryWrapper.WrapperLookup registries) {
        mutate(stack, root -> {
            NbtCompound equipment = root.getCompound(EQUIPMENT_KEY);
            var ops = registries.getOps(NbtOps.INSTANCE);
    ...
    public static float getDropChance(ItemStack stack, String slot) {
        NbtCompound chances = getRoot(stack).getCompound(EQUIPMENT_DROP_CHANCES_KEY);
        return chances.contains(slot) ? chances.getFloat(slot) : 0.0f;
    }

    public static void setDropChance(ItemStack stack, String slot, float chance) {
        mutate(stack, root -> {
            NbtCompound chances = root.getCompound(EQUIPMENT_DROP_CHANCES_KEY);
