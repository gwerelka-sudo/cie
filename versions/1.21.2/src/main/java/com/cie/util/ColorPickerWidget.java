...
public class ColorPickerWidget {
     ...
     * для ChatScreen через обычный @Inject (default-методы интерфейса
     * ParentElement недоступны для Mixin через class-таргет, и
     * interface-mixin на них тоже не работает функционально — см.
     * комментарий в ChatScreenMixin). Драг эмулируется через опрос
     * состояния ЛКМ в render(). Этот флаг — "в прошлом кадре шёл драг
     * внутри пикера, нужно вызвать mouseReleased в этом кадре, раз
     * кнопка мыши теперь отпущена" — устанавливается/читается только
     * миксином.
     */
    public boolean mouseReleasedPending = false;
