...
public class armorStandEditScreen extends Screen {
    ...
    public armorStandEditScreen(ItemStack stack, int handSlot, RegistryWrapper.WrapperLookup registries) {
        ...
        if (player != null && player.getWorld() != null) {
            entity = new ArmorStandEntity(EntityType.ARMOR_STAND, player.getWorld());
        }
    ...
    /**
     * ВЕРСИЯ 1.21.8: подтверждено декомпиляцией реального InventoryScreen
     * .class — высокоуровневый drawEntity(x1,y1,x2,y2,size,scale,mouseX,
     * mouseY,entity) БЕЗУСЛОВНО перезаписывает entity.bodyYaw/setYaw/
     * setPitch внутри себя, целиком выведенные из (center - mouse)/40 —
     * наши entity.setYaw(...)/setPitch(...) снаружи полностью
     * игнорируются (метод их перетирает), а в конце ещё и откатывает
     * исходные значения обратно. Подмена "мыши" на центр (h=i=0) поэтому
     * давала фиксированный угол 180° — превью никогда не крутилось.
     *
     * Используем вместо этого низкоуровневый overload — drawEntity(
     * context, x1, y1, x2, y2, scale, translation, rotation,
     * overrideCameraAngle, entity) — он НЕ трогает entity.setYaw()/
     * setPitch() вообще, принимает готовые Quaternionf напрямую. Формула
     * кватернионов скопирована из высокоуровневого метода один в один,
     * только h/i (там — из дельты курсора) заменены на наши
    ...
    private void drawArmorStandPreview(DrawContext context, int x1, int y1, int x2, int y2, int size, LivingEntity entity) {
        // ИСПРАВЛЕНО: раньше yaw крутился через entity.setYaw()/bodyYaw —
        // это вращает саму сущность в мировых координатах, что для
        // ArmorStandEntity крутит ВСЮ модель целиком, включая base plate
        // (диск в основании, который в реальной игре никогда не
        // поворачивается вместе с "телом" при осмотре — это часть общей
        // модели с фиксированной ориентацией относительно мира). Теперь
        // и yaw, и pitch крутят только КАМЕРУ (через кватернион rotation),
        // а сущность остаётся с фиксированным yaw/pitch — как и должно
        // быть для превью "осматриваем неподвижный объект со всех сторон".
        //
        // Инвертирован знак yaw (было: тянешь вправо — крутится влево).
        // pitchDeg БЕЗ инверсии знака: при переносе pitch с entity.setPitch()
        // на кватернион камеры направление стало противоположным (камера
        // крутится в другую сторону относительно того же знака угла, чем
        // сама сущность) — тянешь вниз, стойка должна "вставать на попа"
        // к камере (наклоняться к нам верхом), не заваливаться назад.
        float pitchDeg = previewPitch;

        Quaternionf rotation = new Quaternionf().rotateZ((float) Math.PI);
        Quaternionf yawRotation = new Quaternionf().rotateY(-previewYaw * ((float) Math.PI / 180F));
        Quaternionf pitchRotation = new Quaternionf().rotateX(pitchDeg * ((float) Math.PI / 180F));
        rotation.mul(yawRotation).mul(pitchRotation);
        ...
        Vector3f translation = new Vector3f(0.0F, entity.getHeight() / 2.0F + scale * entityScale, 0.0F);
        float p = (float) size / entityScale;

        InventoryScreen.drawEntity(context, x1, y1, x2, y2, p, translation, rotation, pitchRotation, entity);
    }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        ...
        // (например, на других версиях/сборках) — делегирует в тот же
        // общий метод, который polling вызывает из render().
    ...
    /**
     * ВЕРСИЯ 1.21.8: общая логика драга панели/превью, вызывается КАЖДЫЙ
     * КАДР из render() пока зажата ЛКМ (см. комментарий в начале render) —
     * а не через отдельные события mouseDragged, которые в этой версии
     * не диспетчеризуются движком для этого класса. Раньше сюда
     * передавались только дельты одного события; теперь считаем дельту
     * сами через lastPreviewDragX/Y (уже существовавшие поля).
     */
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        // ВЕРСИЯ 1.21.8: mouseDragged/mouseReleased НЕ вызываются движком
        // для этого класса (та же причина, что и в ChatScreenMixin —
        // физически не переопределены нигде в цепочке Screen ->
        // AbstractParentElement, чисто default из интерфейса
        // ParentElement — переопределение ниже компилируется, но реально
        // не диспетчеризуется). Поэтому драг эмулируется здесь, через
        ...
        // ВЕРСИЯ 1.21.8: drawStrokedRectangle ещё не существует на этой
        // версии — используем drawBorder (та же сигнатура x,y,width,height,color).
    ...
    private class AxisSlider extends SliderWidget {
        ...
        public boolean mouseReleased(double mouseX, double mouseY, int button) {
            ...
            // ВЕРСИЯ 1.21.8: mouseReleased(double, double, int) — старая
            // сигнатура, действовавшая до рефакторинга ввода 1.21.9
            // (на 1.21.9+ здесь используется mouseReleased(Click)).
            rebuild();
            return result;
        }
