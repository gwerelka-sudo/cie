...
public class ColorPickerWidget {
    ...
    /**
     * ВЕРСИЯ 1.21.8: mouseDragged/mouseReleased не переопределены нигде в
     * цепочке ChatScreen -> Screen -> AbstractParentElement (см. комментарий
     * в ChatScreenMixin), поэтому драг эмулируется через опрос состояния
     * ЛКМ в render(). Этот флаг — "в прошлом кадре шёл драг внутри
     * пикера, нужно вызвать mouseReleased в этом кадре, раз кнопка мыши
     * теперь отпущена" — устанавливается/читается только миксином.
     */
